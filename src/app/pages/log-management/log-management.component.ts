...
import { LogAdvancedSearchModel } from './model/log-advanced-search-model';
...
export class LogManagementComponent implements OnInit {
   ...
   sortByColumn: string;
   advancedSearch: LogAdvancedSearchModel = new LogAdvancedSearchModel();
   ...
   ngOnInit() {
    ...
    onSort(sortField: string) {
        ...
        if (this.sortAscOrder) {
            ...
            this.datas = new List<any>(this.datas).OrderByDescending(x => x[sortField]).ToArray();
        }
    }

    onResetSearch() {
        this.advancedSearch = new LogAdvancedSearchModel();
        this.sortByColumn = '';
